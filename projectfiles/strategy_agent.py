...
class StrategyAgent(DoNothingAgent):
	...
	def decide(self, game):
		...
		def wrapped_approximator(state, next_state):
			if next_state.current_player_win(): return 1e9
			if next_state.current_player_lose(): return -1e9
			return self.function_approximator(state, next_state)

		manager.think(wrapped_approximator)
		return manager.find_best_action(wrapped_approximator)
